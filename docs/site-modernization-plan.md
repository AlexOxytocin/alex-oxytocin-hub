...
Публичный сайт переносится на **Astro 5 + TypeScript strict + static generation**. Это наиболее подходящая модель для контентного сайта: быстрый HTML, нулевая гидрация для обычных компонентов и возможность подключать интерактивность локально там, где она нужна.
