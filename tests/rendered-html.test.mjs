...
test("renders the personal hub with the portrait hero", async () => {
  ...
  assert.match(html, /src="\/assets\/alexey-grishchenko-hero-original-colors-v3\.png\?v=20260819"/);
...
test("static deployment carries both localized portrait pages", async () => {
  const [html, englishHtml, css, ecosystemCss, communityMark, portrait, heroBackground, brandLogo] = await Promise.all([
    ...
    readFile(new URL("../public/assets/alexey-grishchenko-hero-original-colors-v3.png", import.meta.url)),
  ...
  assert.match(css, /\.hero-primary-card\s*\{[^}]*alexey-grishchenko-hero-original-colors-v3\.png/);
  ...
  assert.match(html, /class="hero-portrait"/);
  assert.match(html, /alexey-grishchenko-hero-original-colors-v3\.png\?v=20260819/);
